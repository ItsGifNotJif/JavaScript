{
    ...
    //Task one
    let men = data.filter((person) => person.gender === "Male")
    console.log(men.length)

    //Task Two 
    const peopleIdentifiers = data
        .filter(person => person.car_year > 2000)
        .map(person => person.id);

    console.log(peopleIdentifiers)

    //Task three
    const smallShirts = data
        .filter(person => person.shirt_size === "S" || person.shirt_size === "XS")
        .map(person => person.first_name)

    const sortedShirts = smallShirts.sort((a, b) => {
    ...
    const showIdNameAndSize = data
        .filter(person => person.shirt_size === "S" || person.shirt_size === "XS")
        .map(person => {
            return {
                id: person.id,
                name: person.first_name,
                size: person.shirt_size
            }
        })
        .sort((a, b) => {
