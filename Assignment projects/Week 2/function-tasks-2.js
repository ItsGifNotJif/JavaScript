...
console.log('---');
{
  function mulArrBy2(arr) {
    return arr = arr.map(x => x * 2);
  }
  const newArray = mulArrBy2(numbers);
  console.log({
    numbers,
...
console.log('---');
{
  function filterEquals(arr) {
    const positiveArr = arr.filter(num => num%2===0);
    return positiveArr;
  }
  console.log({
    numbers,
...
{ 
  function arrAbsoluteValues(arr) {
    const positives = arr.map( x => Math.abs(x)); 
    return positives;
  }
...
console.log('---');
{
  function powArrElementsByIndex(arr) {
    ...
    const nthDegree = arr.map( arr[i] => arr[i] ** i);
    return  nthDegree;
  }
  console.log({
    numbers,
...
console.log('---');
{
  function filterNaturals(arr) {
    const x = numbers.filter((numberRes) => numberRes === Math.floor(numberRes) && numberRes >= 1);
    return x;
  }
  console.log({
    numbers,
...
console.log('---');
{
  function filterEverySecond(arr) {
    const filter = arr.filter((element, index) => {
      return index % 2 === 0;
    });
    return filter;
  }
